...
Entity::Entity(
    const float &frame_index, const float &animation_speed,
    const py::Vector2f &direction,
    const std::list<std::shared_ptr<SpriteTexture>> &obstacle_sprites)
    : frame_index(frame_index), animation_speed(animation_speed),
      direction(direction), obstacle_sprites(obstacle_sprites) {}
  ...
  if (!direction.compare("horizontal")) {
    for (const auto &sprite : this->obstacle_sprites) {
      if (sprite->colliderect(this->hitbox_, intersection)) {
        if (this->direction.x > 0) { // move right
          this->hitbox_.transform(intersection.width * -1, 0);
        } else if (this->direction.x < 0) { // move left
    ...
    for (const auto &sprite : this->obstacle_sprites) {
      if (sprite->colliderect(this->hitbox_, intersection)) {
        if (this->direction.y > 0) { // move down
          this->hitbox_.transform(0, intersection.height * -1);
        } else if (this->direction.y < 0) { // move up
