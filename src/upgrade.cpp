...
Upgrade::Upgrade(std::shared_ptr<Player> player)
    : player(player)
{
...
void Upgrade::input()
{
    if (this->can_move)
    {
        if (sf::Keyboard::isKeyPressed(sf::Keyboard::Right) && this->selection_index < (this->attribute_number - 1))
        {
            this->selection_index += 1;
            this->can_move = false;
            this->selection_time.restart();
        }
        else if (sf::Keyboard::isKeyPressed(sf::Keyboard::Left) && this->selection_index >= 1)
        {
            this->selection_index -= 1;
            this->can_move = false;
            this->selection_time.restart();
...
void Item::trigger(std::shared_ptr<Player> player)
{
    ...
    if (player->exp() > upgrade_cost[upgrade_attribute] && upgrade_cost[upgrade_attribute] < upgrade_cost[upgrade_attribute])
    {
        ...
        player->upgrade_cost(upgrade_attribute, upgrade_cost[upgrade_attribute] * 1.4);
    }
