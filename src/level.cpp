    ...
    for (const auto &row : layout) {
      ...
      for (const auto &col : row) {
        ...
        if (!style.compare("boundary")) {
          ...
          auto grass =
              this->random_choice<std::vector<std::shared_ptr<SpriteTexture>>>(
                  graphics["grass"]);
          auto tile =
              std::make_shared<Tile>(sf::Vector2f(x, y), "grass", *grass);
          ...
          auto tile =
              std::make_shared<Tile>(sf::Vector2f(x, y), "object", *sprite);
          this->visible_sprites.push_back(tile);
          this->obstacle_sprites.push_back(tile);
        } else if (!style.compare("entities")) {
          if (!col.compare("394")) {
            this->player = std::make_shared<Player>(
                sf::Vector2f(x, y), this->obstacle_sprites,
                [this]() -> void { this->create_attack(); },
                [this]() -> void { this->destroy_attack(); },
                [this](const std::string &style, int strength, int cost) {
                  this->create_magic(style, strength, cost);
                },
                [this]() -> void { this->destroy_magic(); },
...
void Level::trigger_death_particles(const sf::Vector2f &pos,
...
void Level::player_attack_logic() {
  for (const auto &attack_sprite : this->attack_sprites) {
    auto collision_sprites =
        ::spritecollide(attack_sprite, this->attackable_sprites);
    for (const auto &collision_sprite : collision_sprites) {
      if (!collision_sprite->sprite_type().compare("grass")) {
        ...
        for (auto i = 0; i < range; ++i) {
          auto particle =
              this->animation_player->create_grass_particles(pos - offset);
...
void YSortCameraGroup::custom_draw(std::shared_ptr<Player> player) {
  ...
  this->sprites.sort([](auto a, auto b) -> bool {
