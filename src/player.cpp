...
void Player::get_status() {
  if (this->direction.x == 0 && this->direction.y == 0) {
    if (std::string::npos == this->status.find("idle") &&
        std::string::npos ==
            this->status.find("attack")) { // not idle or attack
      std::stringstream ss;
  ...
  if (this->attacking) {
    ...
    if (std::string::npos == this->status.find("attack")) {
      if (std::string::npos != this->status.compare("idle")) { // found
    ...
    if (std::string::npos != this->status.find("attack")) { // found
...
void Player::animate() {
  ...
  this->get_rect({"center", this->hitbox.center()});
}
void Player::update() {
  this->input();
  this->cooldowns();
